...
const pathAliasOBundle = openTargetsBundle.replace(
  mainBundlePrefix,
...
test("open-target discovery TryExec parser does not shadow the minified path alias", () => {
  withTempDir((tmp) => {
    const result = evaluatePatched(
      pathAliasOBundle,
      tryExecEnv(tmp, { binNames: ["sh", "cursor"] }),
      "codexLinuxDesktopTryExecAvailable(`sh -c 'command -v cursor >/dev/null 2>&1'`)",
    );
